...
<body>
    <div class="container">
        <h1>Đăng kí</h1>
        <form action="?action=addKhachhang" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
            <label>Họ Và Tên:</label>
            <input type="text" name="ten" id="ten">
            <span class="error-message" id="ten-error">Vui lòng nhập họ và tên</span>
            <label>Email:</label>
            <input type="email" name="email" id="email">
            <span class="error-message" id="email-error">Vui lòng nhập email</span>
            <label>Mật Khẩu:</label>
            <input type="password" name="matkhau" id="matkhau">
            <span class="error-message" id="matkhau-error">Vui lòng nhập mật khẩu</span>
            <hr>
            <input type="submit" value="Đăng kí" name="addKhachhang">
        </form>
    </div>
    ...
    <script>
        function validateForm() {
            ...
            if (email.indexOf('@') === -1) {
                emailInput.classList.add('error');
                emailError.textContent = 'Email không hợp lệ. Email phải chứa ký tự "@"';
                emailError.style.display = 'block';
                return false;
            }
